...
export default function BlogPostForm({ post, onClose, onSuccess }: BlogPostFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Failed to create blog post. Please check the console for details.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="mt-2 text-sm text-gray-500">
          ...
          • Estimated reading time: {Math.ceil(content.split(/\s+/).length / 200)} minutes
        </div>
      </div>
      ...
      {formData.status === "SCHEDULED" && (
        <div>
          ...
          <input
            ...
            onChange={(e) => updateField("scheduledFor", new Date(e.target.value).toISOString())}
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          />
        </div>
      )}
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case "author": return renderAuthorTab();
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            ...
            <p className="text-gray-600">Manage your blog content and SEO optimization</p>
          </div>
          <button
            onClick={onClose}

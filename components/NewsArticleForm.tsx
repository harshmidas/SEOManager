...
//         // Ensure SEO dates are properly formatted
//         seoMeta: {
//           ...formData.seoMeta,
//           articlePublishedTime: formData.seoMeta.articlePublishedTime 
//             ? new Date(formData.seoMeta.articlePublishedTime).toISOString()
//             : new Date(formData.publishedAt).toISOString(),
//           articleModifiedTime: formData.seoMeta.articleModifiedTime 
//             ? new Date(formData.seoMeta.articleModifiedTime).toISOString()
//             : new Date().toISOString(),
//         },
//         // Ensure proper formatting for updates if they exist
//         updates: formData.updates?.map(update => ({
//           ...update,
//           timestamp: new Date(update.timestamp).toISOString()
//         })) || [],
//         // Set proper timestamps for created/updated
//         createdAt: article ? formData.createdAt : new Date().toISOString(),
//         updatedAt: new Date().toISOString(),
//       };

//       // Remove id for new articles, include it for updates
//       const { id, ...dataWithoutId } = submitData as any;
//       const finalData = article ? submitData : dataWithoutId;

//       const url = article ? `${API_URL}/${article.id}` : API_URL;
...
//       alert(
//         `Failed to ${article ? 'update' : 'create'} news article. Please check the console for details.`
//       );
//     } finally {
//       setLoading(false);
//     }
...
//           <input
//             type="datetime-local"
//             value={formData.seoMeta.articlePublishedTime?.slice(0, 16) || ""}
//             onChange={(e) =>
//               updateField("seoMeta.articlePublishedTime", new Date(e.target.value).toISOString())
//             }
//             className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500"
//           />
//         </div>
...
//           <input
//             type="datetime-local"
//             value={formData.seoMeta.articleModifiedTime?.slice(0, 16) || ""}
//             onChange={(e) =>
//               updateField("seoMeta.articleModifiedTime", new Date(e.target.value).toISOString())
//             }
//             className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500"
//           />
//         </div>
...
//         <input
//           type="text"
//           value={formData.seoMeta.articleTags.join(", ")}
//           onChange={(e) =>
//             updateField(
//               "seoMeta.articleTags",
//               e.target.value.split(",").map((tag) => tag.trim()).filter(tag => tag.length > 0)
...
//                   : (article ? "Update News Article" : "Create News Article")
//                 }
//               </button>
//             </div>
//           </form>
...
export default function NewsArticleForm({
  ...
  const priorities = ["LOW", "NORMAL", "HIGH", "URGENT", "BREAKING"];
    ...
    try {
      ...
      const submitData = {
        ...
        seoMeta: {
          ...
          metaKeywords: formData.seoMeta.metaKeywords.filter(keyword => keyword.trim() !== ""),
          ...
          articlePublishedTime: formData.seoMeta.articlePublishedTime || formData.publishedAt,
          articleModifiedTime: formData.seoMeta.articleModifiedTime || new Date().toISOString(),
          articleSection: formData.seoMeta.articleSection || formData.category,
          articleTags: formData.seoMeta.articleTags.filter(tag => tag.trim() !== ""),
          noIndex: formData.seoMeta.noIndex,
          structuredData: formData.seoMeta.structuredData || {
            "@context": "https://schema.org",
            "@type": "NewsArticle",
            "headline": formData.title,
            "image": formData.seoMeta.ogImage || formData.featuredImage,
            "author": formData.reporter,
        ...
        relatedNewsIds: formData.relatedNewsIds || [],
        updates: formData.updates || [],
        status: formData.status,
        createdAt: article ? formData.createdAt : new Date().toISOString(),
        updatedAt: new Date().toISOString(),
        createdBy: article ? formData.createdBy : "admin",
        updatedBy: article ? formData.updatedBy : "admin",
        metadata: formData.metadata || {
          language: "en",
          wordCount: content.split(/\s+/).filter(word => word.length > 0).length,
          readingLevel: "Intermediate",
          geoTarget: "US",
          topicCluster: formData.category
        }
      };

      console.log("Submitting data:", JSON.stringify(submitData, null, 2));

      const url = article ? `${API_URL}/${formData.id}` : API_URL;
      const method = article ? "PUT" : "POST";
  ...
  // ... (keep all the other functions like updateField, addMediaUrl, removeMediaUrl the same)
  ...
  // ... (keep all the render functions exactly the same - basicTab, contentTab, mediaTab, seoTab, settingsTab)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input
            type="datetime-local"
            value={formData.seoMeta.articlePublishedTime?.slice(0, 16) || formData.publishedAt.slice(0, 16)}
            onChange={(e) =>
              updateField("seoMeta.articlePublishedTime", new Date(e.target.value).toISOString())
            }
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500"
        ...
        <div>
          ...
          <input
            type="datetime-local"
            value={formData.seoMeta.articleModifiedTime?.slice(0, 16) || new Date().toISOString().slice(0, 16)}
            onChange={(e) =>
              updateField("seoMeta.articleModifiedTime", new Date(e.target.value).toISOString())
            }
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500"
      ...
      <div>
        ...
        <input
          type="text"
          value={formData.seoMeta.articleSection || formData.category}
          onChange={(e) => updateField("seoMeta.articleSection", e.target.value)}
          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500"
        />
      </div>

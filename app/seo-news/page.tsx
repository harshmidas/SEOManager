...
export default function SeoNewsPage() {
  ...
  useEffect(() => {
    ...
    if (priorityFilter !== "all") {
      filtered = filtered.filter(article => article.priority === priorityFilter);
    }

    if (breakingNewsFilter !== "all") {
      filtered = filtered.filter(article => 
        breakingNewsFilter === "breaking" ? article.breakingNews : !article.breakingNews
      );
    }

    setFilteredArticles(filtered);
  }, [searchTerm, statusFilter, categoryFilter, priorityFilter, breakingNewsFilter, newsArticles]);
  ...
  const totalViews = newsArticles.reduce((sum, article) => sum + article.viewCount, 0);
  const totalShares = newsArticles.reduce((sum, article) => sum + article.shareCount, 0);
  const publishedCount = newsArticles.filter(article => article.published).length;
  const breakingNewsCount = newsArticles.filter(article => article.breakingNews).length;
  const urgentCount = newsArticles.filter(article => article.priority === "URGENT").length;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            <div>
              ...
              <p className="text-gray-600 mt-2">Manage breaking news and real-time content</p>
            </div>
            <button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {filteredArticles.some(article => article.breakingNews && article.published) && (
          <div className="mb-6">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm opacity-90">
                    {filteredArticles.filter(article => article.breakingNews && article.published).length} 
